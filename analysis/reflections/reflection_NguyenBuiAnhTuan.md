...
| Failure analysis / Error Tree | M4 | `failure_analysis()` + `DIAGNOSTIC_TREE` | Map `min(4 metrics)` → nguyên nhân gốc → fix. Rất hiệu quả: 4/5 câu tệ nhất có worst metric = faithfulness. Đã bổ sung nhánh nhận diện answer từ chối để phân biệt context thiếu mắt nối với hallucination |
